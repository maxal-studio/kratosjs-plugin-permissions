...
import {
  createAdminPermissionsEntity,
  type IAdminPermissions,
} from "./entities/AdminPermissions";
...
export class PermissionsPlugin extends Plugin {
  ...
  async register(panel: Panel): Promise<void> {
    ...
    panel.registerMetadataFilterHook(
      async (metadata: PanelMetadata, user?: AuthUser) => {
        if (!user || !user.role) {
    ...
    panel.registerFormSchemaFilterHook(
      async (schema: any, resourceSlug: string, user?: AuthUser) => {
        if (!user || !user.role) {
    ...
    panel.registerTableSchemaFilterHook(
      async (schema: any, resourceSlug: string, user?: AuthUser) => {
        if (!user || !user.role) {
          // No user or role - return empty schema (deny all)
          return {
    ...
    panel.registerPageBlocksFilterHook(
      async (blocks: any[], pageSlug: string, user?: AuthUser) => {
        if (!user || !user.role) {
    ...
    panel.registerPageAccessCheckHook(
      async (pageSlug: string, user?: AuthUser) => {
        if (!user || !user.role) {
          return false;
        }
    ...
    panel.registerCapabilitiesFilterHook(
      async (
        ...
        return {
          canCreate:
            capabilities.canCreate && resourcePermission.create === true,
          canEdit: capabilities.canEdit && resourcePermission.update === true,
          canDelete:
            capabilities.canDelete && resourcePermission.delete === true,
    ...
    panel.registerActionAccessCheckHook(
      async (actionName: string, resourceSlug: string, user?: AuthUser) => {
   ...
   */
  private patchUserEntity(
    panel: Panel,
    adminPermissions: EntitySchema<IAdminPermissions>,
  ): void {
