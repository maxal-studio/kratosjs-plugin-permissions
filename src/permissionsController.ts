...
function denormalizePermissions(permissions: any): any {
	...
	if (denormalized.pages && typeof denormalized.pages === 'object') {
		denormalized.pages = Object.entries(denormalized.pages).reduce((acc, [pageSlug, pagePerm]: [string, any]) => {
...
export async function getPermissions(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
	try {
		const em = req.panel!.getEm();
		const role = (req.query.role as string) || 'editor';
		const permissions = await em.findOne<IAdminPermissions>(getPermissionsEntity(), { role } as any);
...
export async function savePermissions(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
	try {
		const em = req.panel!.getEm();
		...
		res.status(500).json({ message: error.message || 'Failed to save permissions' });
	}
}

export async function getAllRoles(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
	try {
		const em = req.panel!.getEm();
		const permissions = await em.find<IAdminPermissions>(getPermissionsEntity(), {});
...
export async function getStructure(req: KratosJsRequest, res: KratosJsResponse): Promise<void> {
	try {
